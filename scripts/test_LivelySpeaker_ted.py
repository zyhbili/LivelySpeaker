...
def infer_from_testloader(sample_fn, model, args, test_dataloader, guidance_param = 2.5, speaker_model = None, eta = 0, skip_steps = 0):
    ...
    for i, data in enumerate(test_dataloader):
        ...
        texts = aux_info["sentence"]
        batch = {
            "x": vec_seq.reshape(B, T ,9,3).permute(0,2,3,1),
            'mask':torch.ones(B,34).to(device).bool(),
            'clip_text': texts,
        }
        texts = clip.tokenize(batch['clip_text']).cuda()
        features = clip_model.encode_text(texts)
        batch['z'] = features
        ret = SAG.decoder(batch)
        # import pdb;pdb.set_trace()

        decoded_motions = ret['output']
...
if __name__== "__main__":
    ...
    with torch.no_grad():
        ...
        t = infer_from_testloader(sample_fn, model, args, test_data_loader, guidance_param = 1, speaker_model=speaker_model, eta = 0, skip_steps= skip_steps)
        result_list.append(t)
        print(t)

        t = infer_from_testloader(sample_fn, model, args, test_data_loader, guidance_param = 1.5, speaker_model=speaker_model, eta = 0, skip_steps= skip_steps)
        result_list.append(t)
        print(t)
