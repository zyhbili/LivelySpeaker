...
def compute_huber(out_dir_vec, target_poses):
    beta = 0.1
...
class LossType(enum.Enum):
    ...
    HUBER = enum.auto()  # like KL, but rescale to estimate the full VLB
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
            ...
            noisy_inpainted_motion = self.q_sample(inpainted_motion, t-1) if (t[0] > 0) else inpainted_motion
            model_output = (model_output * ~inpainting_mask) + (noisy_inpainted_motion * inpainting_mask)
            # import pdb;pdb.set_trace()
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            if const_noise:
                img = img[[0]].repeat(img.shape[0], 1, 1, 1)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            all_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
            model_output = all_output['output']
            ...
            if 'z_mu' in all_output.keys():
                z_mu = all_output['z_mu']
                z_logvar = all_output['z_logvar']
                terms["kld"] = -0.5 * torch.mean(1 + z_logvar - z_mu.pow(2) - z_logvar.exp())
            ...
            terms["rot_mse"] = compute_huber(target, model_output) # mean_flat(rot_mse)

            if self.lambda_vel > 0.:
                target_vel = (target[..., 1:] - target[..., :-1])
                model_output_vel = (model_output[..., 1:] - model_output[..., :-1])
                terms["vel_mse"] = compute_huber(target_vel, # Remove last joint, is the root location!
                                                  model_output_vel)  # mean_flat((target_vel - model_output_vel) ** 2)
            # import pdb;pdb.set_trace()
            if 'z_mu' in all_output.keys():
                z_mu = all_output['z_mu']
                z_logvar = all_output['z_logvar']
