...
mean_pose = np.load('/your/path/to/BEAT/bvh_mean.npy')
std_pose = np.load('/your/path/to/BEAT/bvh_std.npy')  

def build_data_with_beat(mode = "train"):
    preloaded_dir = f"/your/path/to/BEAT/{mode}/bvh_rot_2_4_6_8_cache"
    lmdb_env = lmdb.open(preloaded_dir, readonly=True, lock=False)
    with lmdb_env.begin() as txn:
        n_samples = txn.stat()['entries']
    map_size = 1024 * 50  # in MB
    map_size <<= 20  # in B
    out_lmdb_dir = f"/your/path/to/BEAT/{mode}/my6d_bvh_rot_2_4_6_8_cache"
