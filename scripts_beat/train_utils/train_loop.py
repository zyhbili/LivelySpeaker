...
class TrainLoop:
    ...
    def run_loop(self):
        for epoch in range(self.num_epochs):
            ...
            for batch in tqdm(self.data):
                ...
                rep_dim = 6
                sub_pose = tar_pose
                motion = sub_pose.reshape(sub_pose.shape[0],sub_pose.shape[1],-1,rep_dim).permute(0,2,3,1)
                # mel = spectrogram.to(self.device).float()
                n_frames = tar_pose.shape[1]
                vid_indices = vid.to(self.device).long()


                in_word = in_word.to(self.device).long()
                cond = {'y': {'mask': torch.ones(sub_pose.shape[0],sub_pose.shape[1]).to(self.device).bool()
                , 'lengths': torch.ones(sub_pose.shape[0],sub_pose.shape[1]).to(self.device)*n_frames, 'text':aux_info["sentence"],
                    'vid_indices': vid_indices, 'audio_input':in_audio.to(self.device).float(), 'text_padded': in_word, 'emo': emo.to(self.device).long(),
                    'origin_x': motion.clone().to(self.device)}}
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            loss = (losses["loss"] * weights).mean() + 0* losses.get('kld',0.0) # there are only 4 speaker in BEAT, we remove the KL divergence
            log_loss_dict(
